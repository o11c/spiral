...
Mesh::Mesh(FlatProgram *fp, ShadeProgram *sp, TextureProgram *tp)
: flat_program(fp)
, shade_program(sp)
, texture_program(tp)
, mesh_rings(false)
...
vec3 Mesh::SE(Radians u, Radians v)
{
...
vec3 Mesh::ST(Radians u, Radians v)
{
...
vec3 Mesh::SN(Radians u, Radians v)
{
...
#define INDEX(i, j) ((i) * (M + 1) + (j))
    ...
    char filename[256];
    sprintf(filename, "data/mesh-%.10e-%.10e,%.10e-%d,%d.mesh",
            tor ? d : 0.0f,
            em, en,
            dm, dn);
